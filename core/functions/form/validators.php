...
/**
 * Checks if the input contains two words seperated by a space
 *
 * @param string $field_value
 * @param array $field
 * @return boolean
...
function validate_field_contains_space(string $field_value, array &$field): bool
{
    if (str_word_count(trim($field_value)) < 2) {
        $field['error'] = 'ERROR FIELD MUST CONTAIN A SPACE';
        return false;
    }
...
/**
 * Check if input is in valid URL format
 *
 * @param string $field_value
 * @param array $field
 * @return boolean
 */
function validate_field_is_url(string $field_value, array &$field): bool
{
